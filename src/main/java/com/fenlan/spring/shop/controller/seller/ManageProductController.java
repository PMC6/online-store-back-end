...
public class ManageProductController {
    ...
    public ResponseEntity<ResponseFormat> productNum(){
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        ...
        try {
            ...
            return new ResponseEntity<>(new ResponseFormat.Builder(new Date(), HttpStatus.OK.value())
                    ...
                    .message("amount success")
                    .path(request.getServletPath())
                    .data(productService.amountProductOfShop(shop.getId(), ""))
                    .build(), HttpStatus.OK);
        }catch (Exception e) {
            return new ResponseEntity<>(new ResponseFormat.Builder(new Date(), HttpStatus.INTERNAL_SERVER_ERROR.value())
                    .error("amount error")
    ...
    public ResponseEntity<ResponseFormat> judgeWhetherAppliedToMail(@RequestParam("productId") Long productId){
        try {
            return new ResponseEntity<>(new ResponseFormat.Builder(new Date(), HttpStatus.OK.value())
                    ...
                    .path(request.getServletPath())
                    .data(productService.judgeWhereAppliedToMail(productId))
                    .build(), HttpStatus.OK);
        }catch (Exception e){
            return new ResponseEntity<>(new ResponseFormat.Builder(new Date(), HttpStatus.INTERNAL_SERVER_ERROR.value())
                    .error("query failed")
