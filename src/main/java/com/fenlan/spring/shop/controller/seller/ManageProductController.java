...
public class ManageProductController {
     ...
     * 查看正在商场主页展示的商品
     */
    @GetMapping("/product/advertisement/search/applied")
    public ResponseEntity<ResponseFormat> getByApply(@RequestParam("page") int page,
                                                     @RequestParam("size") int size,
                                                     @RequestParam("shopName") String shopName){
        try {
            Shop shop = shopService.findByName(shopName);
            if (shop == null) throw new Exception("There doesn't has this shop");
            return new ResponseEntity<>(new ResponseFormat.Builder(new Date(), HttpStatus.OK.value())
                    .error(null)
                    .message("find the products")
                    .path(request.getServletPath())
                    .data(productService.listByApplied(page, size, shop.getId()))
                    .build(), HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(new ResponseFormat.Builder(new Date(), HttpStatus.INTERNAL_SERVER_ERROR.value())
                    .error("find failed")
